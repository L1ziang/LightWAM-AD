    ...
    *,
...
) -> dict[tuple[int, str], dict[str, set[Path]]]:
    # The cache suffix is a dataset contract rather than an incidental spelling of
    # the Hugging Face model id.  Keeping it in the grouping key makes precompute
    # and dataset lookup agree even when a project uses a human-readable alias
    # such as ``wan21t2v1.3b``.
    prompt_groups: dict[tuple[int, str], dict[str, set[Path]]] = defaultdict(
    ...
    for node_path, node in _iter_navsim_nodes(data_cfg):
        ...
        if not dataset.use_dynamic_prompt:
            ...
            prompt_groups[(dataset.context_len, dataset.text_encoder_id)][prompt].add(cache_dir)
        ...
        logger.info(
            "Scanned `%s`: samples=%d cache_dir=%s context_len=%d text_encoder_id=%s "
            "dynamic_prompt=%s prompts(before dedup)=%d",
            node_path,
            len(dataset.tokens),
            cache_dir,
            dataset.context_len,
            dataset.text_encoder_id,
...
def main(cfg: DictConfig):
    ...
    derived_enc_id = _model_id_to_enc_id(model_id)
    ...
    configured_enc_ids = sorted({enc_id for _, enc_id in prompt_groups})
    if configured_enc_ids != [derived_enc_id]:
        logger.info(
            "Using dataset cache encoder id(s) %s (model-derived id is `%s`).",
            configured_enc_ids,
            derived_enc_id,
        )

    for context_len, enc_id in sorted(prompt_groups):
        prompts_to_dirs = prompt_groups[(context_len, enc_id)]
        tokenizer = HuggingfaceTokenizer(
        ...
        if (not is_distributed) or rank == 0:
            cache_dir_count = len(
                {cache_dir for dirs in prompts_to_dirs.values() for cache_dir in dirs}
            )
            logger.info(
        ...
        _encode_group(
            prompts_to_dirs=prompts_to_dirs,
