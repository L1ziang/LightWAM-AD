...
def _load_model_checkpoint(model: torch.nn.Module, ckpt: str) -> None:
    ...
    if payload.get("format") == "lightwam_ad_v1":
        if not hasattr(model, "load_checkpoint"):
            raise TypeError("The configured model cannot load a LightWAM-AD checkpoint.")
        model.load_checkpoint(ckpt, optimizer=None)
        logging.info("Loaded LightWAM-AD checkpoint with strict PEFT topology validation.")
        return
