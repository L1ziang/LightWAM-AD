"""LightWAM-AD trajectory prediction on NAVSIM-v2 splits.

Writes one 4-second, 2-Hz trajectory to ``<token>.npy`` in the current ego frame.
Uses the v2 SceneLoader and shared checkpoint, preprocessing and normalization
helpers. PDM scoring runs separately. Launch with ``run_predict_navsim_v2.sh``;
tokens are sharded across torchrun ranks.
"""
...
def _build_scene_loader(cfg: DictConfig) -> SceneLoader:
    ...
    # Inference consumes only the current frame.  Loading cam_f0=True would read every
    # history/future image in the Scene and distort both I/O cost and latency measurements.
