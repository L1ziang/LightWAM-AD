"""LightWAM-AD prediction on NAVSIM-v2 ``navhard_two_stage``.

Covers original stage-one and synthetic stage-two scenes. Each ``<token>.npy``
contains eight ``(x, y, heading)`` poses in the current ego frame for the v2
NpyTrajectoryAgent scorer. Launch with ``run_predict_navhard.sh``; tokens are
sharded across torchrun ranks.
"""
...
def _build_scene_loader(cfg: DictConfig) -> SceneLoader:
    ...
    # LightWAM-AD inference consumes only the current front frame.  Restricting the
    # SensorConfig avoids reading unused history/future JPEGs for original and synthetic scenes.
    current_frame_index = int(scene_filter.num_history_frames) - 1
    if current_frame_index < 0:
        raise ValueError("scene_filter.num_history_frames must be positive.")
    sensor_config = SensorConfig(
        cam_f0=[current_frame_index], cam_l0=False, cam_l1=False, cam_l2=False,
...
def _load_text_context(
    ...
    if cache_dir is None:
        raise ValueError("`text_embedding_cache_dir` is required for cached text context.")
    import hashlib

    hashed = hashlib.sha256(prompt.encode("utf-8")).hexdigest()
    cache_path = Path(cache_dir) / f"{hashed}.t5_len{context_len}.{text_encoder_id}.pt"
...
def predict(cfg: DictConfig, model: torch.nn.Module, loader: SceneLoader, out_dir: Path) -> dict[str, Any]:
    ...
    text_encoder_id = str(data_cfg.get("text_encoder_id", "wan22ti2v5b"))
    ...
    for token in progress:
        ...
        try:
            ...
            if prompt not in context_cache:
                context_cache[prompt] = _load_text_context(
                    text_cache_dir, context_len, prompt, text_encoder_id=text_encoder_id
                )
