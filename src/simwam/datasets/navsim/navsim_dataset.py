...
class NavSimVideoDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        expected_action_horizon = (self.num_frames - 1) * self.frame_stride
        if self.future_action_horizon != expected_action_horizon:
            raise ValueError(
                "NAVSIM video/action horizons must cover the exact same physical interval: "
                "expected `future_action_horizon == (num_frames - 1) * frame_stride`, "
                f"got {self.future_action_horizon} != ({self.num_frames} - 1) * {self.frame_stride} "
        ...
        # SensorConfig accepts explicit scene-frame indices.  Restrict sensor I/O to
        # the frames that will actually enter the video tensor instead of loading the
        # complete NAVSIM scene window.  Besides avoiding unnecessary image reads,
        # this is required for partial sensor releases where unused boundary frames
        # (for example indices 12 and 13 in navtrain) are not present on disk.
    ...
    def _build_sensor_config(
        ...
        load_indices = sorted({int(frame_idx) for frame_idx in frame_indices})
        if not load_indices:
            raise ValueError("`frame_indices` must contain at least one scene-frame index.")
        if load_indices[0] < 0:
            raise ValueError(f"`frame_indices` must be non-negative, got {load_indices}.")
    ...
    def build_prompt_fixed(
        ...
        if not use_dynamic_prompt:
            return (
                ...
                "Given the current front-camera observation and current ego state, "
